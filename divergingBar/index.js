

function enterFn(e){
  
  //rects
  e.append('rect')
    .attrs({
      class: 'annual-growth',
      x: (d) => xScale(Math.min(0, d.annual_growth)),
      y: getScaledCountry,
      height: yScale.bandwidth(),
      width: (d) => Math.abs(xScale(d.annual_growth) - xScale(0)),
    })
    .style('fill', getColoredGrowth);


  //labels
  e.append("g")
    .attr("class", "labelGWrapper")
    .append("text")
    .attrs({
      "class": "bar-label",
      "x": xScale(0),
      "y": getScaledCountry,
      "dx": d => d.annual_growth < 0 ? state.labelMargin : -state.labelMargin,
      "dy": yScale.bandwidth(),
      "text-anchor": d => d.annual_growth < 0 ? "start" : "end"
    })
    .text(getCountry)
    .style("fill", function(d) {
       if (d.country == "European Union") {
         return "blue";
       }
    });
}

/*
  Element Creation
*/
const svg = d3
  .select("#chartDiv")
  .append("svg")
  .attrs({ width, height });

const gWrapper = svg.append("g")
  .attrs({
    "transform": `translate(${state.margin.left},${state.margin.top})`,
    'class': 'gWrapper'
  });

const legend = gWrapper.append("g")
  .attr("class", "legend");

/*

  Scales

*/ 
const xScale = d3.scaleLinear().range([0, wLM]);
const xScaleObj = d3.axisBottom(xScale)
const colorScale = d3.scaleSequential(d3.interpolatePRGn);

const yScale = d3.scaleBand()
	.range([hLM, 0])
	.padding(0.1);


/*

  Legend Deets

*/ 
legend.append("text")
  .attrs({
    "x": wLM - state.legendRightMargin,
    "y": state.margin.top,
    "text-anchor": "end"
  })
	.text("European Countries");

legend.append("text")
  .attrs({
    "x": wLM - state.legendRightMargin,
	  "y": state.margin.top + 20,
    "text-anchor": "end",
  })
	.style("opacity", 0.5)
	.text("by 2016 Population Growth Rate (%)");


async function runIt() {
  const data = await d3.json('./data.json')
  //get annual-growth max
  const max = d3.max(data, getAnnualGrowth);

  //set scale domains from data
  yScale.domain(data.map(getCountry));
  xScale.domain(d3.extent(data, getAnnualGrowth));
  colorScale.domain([-max, max]);

  //build axes
  const yAxis = appendG({
    parent: gWrapper,
    className: 'y-axis',
    trans: 'translate(' + xScale(0) + ',0)'
  })
  
  const yAxisLine = yAxis.append('line').attrs({
    y1: 0,
    y2: hLM,
  });

  const xAxis = appendG({
    parent: gWrapper,
    className: 'x-axis',
    trans: 'translate(0,' + (hLM + state.xAxisMargin) + ')'
  }).call(xScaleObj);


  let dataJoin = gWrapper.selectAll('rect').data(data);
  dataJoin.join(enterFn);
}

try {
  runIt()
  } catch (error) {
    console.log('GLOBAL')
    console.log(error)
    
  }