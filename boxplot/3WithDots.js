...
const prepData = (srcData) => {
    ...
    return {nested: stats, rootData: srcData}
}
...
const enterCircles = e => {
	let jitterWidth = 50;
	e.append("circle")
    .attr("cx", d => (state.xScale(d.Species) - jitterWidth/2 + Math.random()*jitterWidth ) + (state.xScale.bandwidth() / 2))
    .attr("cy", function(d){return(state.yScale(d.Sepal_Length))})
    .attr("r", 4)
    .style("fill", "white")
...
d3.csv('https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/iris.csv').then(prepData).then(resObj => {
	...
	let circleDataJoin = gWrapper.selectAll('circle')
	.data(rootData)

	circleDataJoin.join(enterCircles)
